...
// maps for the signs in a 4x4 grid layout
const uint8_t sign_cols[12] PROGMEM = {1, 2, 3, 3, 3, 3, 2, 1, 0, 0, 0, 0};
const uint8_t sign_rows[12] PROGMEM = {0, 0, 0, 1, 2, 3, 3, 3, 3, 2, 1, 0};
...
void loop () {
  ...
  display.drawRect(32, 16, 64, 32, SH110X_WHITE);
  
  display.drawLine(32, 0, 32, 16, SH110X_WHITE);   
  display.drawLine(32, 48, 32, 64, SH110X_WHITE);
  display.drawLine(64, 0, 64, 16, SH110X_WHITE);   
  display.drawLine(64, 48, 64, 64, SH110X_WHITE);
  display.drawLine(96, 0, 96, 16, SH110X_WHITE);   
  display.drawLine(96, 48, 96, 64, SH110X_WHITE);
  
  display.drawLine(0, 16, 32, 16, SH110X_WHITE);   
  display.drawLine(96, 16, 128, 16, SH110X_WHITE);
  display.drawLine(0, 32, 32, 32, SH110X_WHITE);   
  display.drawLine(96, 32, 128, 32, SH110X_WHITE);
  display.drawLine(0, 48, 32, 48, SH110X_WHITE);   
  display.drawLine(96, 48, 128, 48, SH110X_WHITE);

  // zodiac signs in proper boxes on left hand side
  for (int sign = 0; sign < 12; sign++) {
    int col = pgm_read_byte(&sign_cols[sign]);
    int row = pgm_read_byte(&sign_rows[sign]);
    int bx = col * 32;
    int by = row * 16;

    unsigned char* zGlyphPtr = (unsigned char*)pgm_read_ptr(&(zodiacGlyphs[sign]));
    for (int c = 0; c < 7; c++) {
      unsigned char colData = pgm_read_byte(&zGlyphPtr[c]);
      for (int r = 0; r < 7; r++) {
        if (colData & (1 << r)) {
          display.drawPixel(bx + 2 + c, by + 4 + r, SH110X_WHITE);
        }
      }
    }
  }

  // populate planets within
  // automatically handles multiple planets in each sign
  ...
  for (int it = 0; it < 7; it++) {
    ...
    // 2x3 mini-grid on the right side of the box for planets
    int slot = planet_slots[sign]++;
    int p_col = slot % 3; 
    int p_row = slot / 3; 
    ...
    for (int c = 0; c < 5; c++) {
      unsigned char colData = pgm_read_byte(&pGlyphPtr[c]);
      for (int r = 0; r < 7; r++) {
        if (colData & (1 << r)) {
          display.drawPixel(px + c, py + r, SH110X_WHITE);
        }
      }
    }
  }
