...
void loop () {
  ...
  for (int it = 0; it < 7; ++it) {
    // print 4 planets on each half of the screen
    int start_x = 0;
    int y_adj = 0;
    if (it > 3) {
      start_x = 64;
    ...
    // planet symbol
    // this odd work around is so we can read and print a variable image array correctly
    unsigned char* glyphPtr = (unsigned char*)pgm_read_ptr(&(planetGlyphs[it]));
    for (int col = 0; col < 5; col++) {
      unsigned char colData = pgm_read_byte(&glyphPtr[col]);
      for (int row = 0; row < 7; row++) {
        if (colData & (1 << row)) {
          display.drawPixel(start_x + col, 12*(it+y_adj)+ 15+ row, SH110X_WHITE);
        }
      }
    }

    // degree and minutes and sign glyph
    display.setCursor(start_x + 10, 12 * (it + y_adj) + 15);
    if (isnan(obsecl[it]) || isinf(obsecl[it])) {
      display.print(F("ERR"));
    } else {
      print_dec_to_sexa(display, obsecl[it]);
    }
  }
